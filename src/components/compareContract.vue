...
<template>
  <div class="compare">
    <el-dialog style="text-align: left" title="编辑信息" :visible.sync="dialogVisible" width="1100px" custom-class="setheight">
      ...
      <img style="float: left;margin-top: 300px;" src="./../assets/copy.png"/>
      <div style="width: 500px;float: left">
        <div style="height: 40px;line-height: 40px;font-size: 12px;text-align: right;color:#999999">编号：M0001</div>
        <div>
          <!--进口商-->
...
//          return time.getTime() > new Date(2017, 11, 30) || time.getTime() < new Date(2017, 11, 11);//注意是||不是&&
  ...
  mounted () {
    this.$root.GOEVENT.$on('COMPARE_CONTRACT', (param) => {
      console.log('2', param.contract_id)
      this.dialogVisible = true
      this.contractEdit(param.contract_id) // 请求合同数据
    })
  },
  computed: {
    okclick: function () {
...
//      console.log('price',price)
    ...
    contractEdit (id) {
      let data = {contract_id: id, isLike: false}
      console.log('jinlai')
      window.GO_UTIL.ajax('get', 'http://192.168.141.116:8090/contract/iron/detail', data, (err, result) => {
        if (!result.error) {
